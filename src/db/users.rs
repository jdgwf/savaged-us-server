...
use savaged_libs::user::{LoginToken, User};
use uuid::Uuid;
...
pub async fn create_login_token(
    ...
    let new_token = user.id.to_string()
        + &"x".to_owned()
        + &Uuid::new_v4().to_string()
        + &"-".to_owned()
        + &Uuid::new_v4().to_string();

    login_tokens.push(LoginToken {
...
pub async fn get_user(pool: &Data<Pool>, user_id: u32) -> Option<User> {
    match pool.get_conn().await {
        Ok(mut conn) => {
            ...
            match found_user_result {
                Some(row) => {
                    let mut user = make_user_from_row(row, "".to_owned()).await;

                    let mut new_count = 0;
                    for msg in &get_notifications_for_user(pool, user.id).await {
                        if msg.read < 1 {
                            new_count += 1;
                        }
                    }
...
// pub fn get_remote_user_public_info(pool: &Data<Pool>, user_id: u32) -> Option<User> {
//     match pool.get_conn().await {
//         Ok(mut conn) => {
//             let found_user_result: Option<Row> = conn
//                 .exec_first(
...
pub async fn admin_get_users(pool: &Data<Pool>, paging_params: Json<FetchAdminParameters>) -> Vec<User> {
    ...
    data_query = data_query
        + admin_filter_where_clause(USER_SEARCH_FIELDS, &paging_params, false, false).as_str();
...
pub async fn admin_get_users_paging_data(
    pool: &Data<Pool>,
    ...
    match &token {
        Some(token_match) => {
            ...
            if token_match.len() < 75 {
                println!(
                    "get_user_from_login_token token length too small! {}",
                    &token_match
                );
                return None;
            }
...
pub async fn get_user_from_api_key(
    ...
    match pool.get_conn().await {
        Ok(mut conn) => {
            ...
            match found_user_result {
                Some(row) => {
                    let user = make_user_from_row(row, "".to_owned()).await;
                    return Some(user);
                }
                None => {
                    return None;
...
pub async fn get_remote_user(
    ...
    if token != None && !token.as_ref().unwrap().is_empty() {
        let token_user_result =
            get_user_from_login_token(&pool, token.to_owned(), request.clone()).await;
        ...
        if api_key != None && !api_key.as_ref().unwrap().is_empty() {
            let api_key_result =
                get_user_from_api_key(&pool, api_key.unwrap().to_owned(), request.clone()).await;
            ...
            match user_id_option {
                Some( user_id ) => {
                    ...
                    if user_id > 0 {
                        ...
                        let user_option  = get_user(&pool, user_id).await;
                        match user_option {
                            Some( user ) => {
                                return Some(_update_user_last_seen(
                                    &pool,
...
pub async fn update_user(pool: &Data<Pool>,user: User) -> u64 {
    ...
    match pool.get_conn().await {
        Ok(mut conn) => {
            ...
            conn.exec_drop(sql, params).await.unwrap();
...
pub async fn username_available(pool: &Data<Pool>,user: User, username: String) -> bool {
    match pool.get_conn().await {
        Ok(mut conn) => {
            ...
            let rows: Option<Row> = conn.exec_first(sql, params).await.unwrap();
            match rows {
                Some(_row) => {
                    return false;
                }
...
pub async fn save_username(pool: &Data<Pool>,user: User, username: String) -> u64 {
    match pool.get_conn().await {
        Ok(mut conn) => {
            let sql = "update `users` SET
            `username` = :username
...
pub async fn update_password(pool: &Data<Pool>,user: User, new_password: Option<String>) -> u64 {
...
async fn _update_user_last_seen(
    pool: &Data<Pool>,
    user: User,
    the_token: String,
    user_agent: String,
...
pub async fn make_user_from_row(mut row: Row, prefix: String) -> User {
    let mut login_tokens_string = "".to_string();
    let login_tokens_string_opt = row
        .take_opt((prefix.to_owned() + &"login_tokens").as_str())
        .unwrap();
    ...
    let login_tokens: Vec<LoginToken> =
        serde_json::from_str(login_tokens_string.as_str()).unwrap_or(Vec::new());

    let mut created_by = 0;
    let created_opt = row
        .take_opt((prefix.to_owned() + &"created_by").as_str())
        .unwrap();
    match created_opt {
        Ok(val) => {
            created_by = val;
    ...
    let updated_opt = row
        .take_opt((prefix.to_owned() + &"updated_by").as_str())
        .unwrap();
    match updated_opt {
        Ok(val) => {
    ...
    let deleted_opt = row
        .take_opt((prefix.to_owned() + &"deleted_by").as_str())
        .unwrap();
    match deleted_opt {
        Ok(val) => {
            deleted_by = val;
    ...
    let share_bio_opt = row
        .take_opt((prefix.to_owned() + &"share_bio").as_str())
        .unwrap();
    match share_bio_opt {
        Ok(val) => {
            share_bio = val;
    ...
    let hidden_banners_opt = row
        .take_opt((prefix.to_owned() + &"hidden_banners").as_str())
        .unwrap();
    ...
    let profile_image_opt = row
        .take_opt((prefix.to_owned() + &"profile_image").as_str())
        .unwrap();
    ...
    let timezone_opt = row
        .take_opt((prefix.to_owned() + &"timezone").as_str())
        .unwrap();
    match timezone_opt {
        Ok(val) => {
            timezone = val;
    ...
    let last_seen_ip_opt = row
        .take_opt((prefix.to_owned() + &"last_seen_ip").as_str())
        .unwrap();
    ...
    let discord_id_opt = row
        .take_opt((prefix.to_owned() + &"discord_id").as_str())
        .unwrap();
    ...
    let user = User {
        ...
        first_name: row
            .take((prefix.to_owned() + &"first_name").as_str())
            .unwrap(),
        ...
        is_admin: row
            .take((prefix.to_owned() + &"is_admin").as_str())
            .unwrap(),
        is_developer: row
            .take((prefix.to_owned() + &"is_developer").as_str())
            .unwrap(),
        is_premium: row
            .take((prefix.to_owned() + &"is_premium").as_str())
            .unwrap(),
        last_name: row
            .take((prefix.to_owned() + &"last_name").as_str())
            .unwrap(),
        last_seen_ip: last_seen_ip,
        last_seen_on: mysql_row_to_chrono_utc(&mut row, "last_seen_on"), // row.take( ( prefix.to_owned() + &"last_seen_on").unwrap(),
        lc_wildcard_reason: row
            .take((prefix.to_owned() + &"lc_wildcard_reason").as_str())
            .unwrap(),
        ...
        notify_email: row
            .take((prefix.to_owned() + &"notify_email").as_str())
            .unwrap(),
        image_url: "".to_string(), // row.take( ( prefix.to_owned() + &"image_url").unwrap(),
        number_years: row
            .take((prefix.to_owned() + &"number_years").as_str())
            .unwrap(),
        partner_id: row
            .take((prefix.to_owned() + &"partner_id").as_str())
            .unwrap(),
        paypal_payment_id: row
            .take((prefix.to_owned() + &"paypal_payment_id").as_str())
            .unwrap(),
        ...
        share_bio: share_bio,
        share_display_name: row
            .take((prefix.to_owned() + &"share_display_name").as_str())
            .unwrap(),
        share_show_profile_image: row
            .take((prefix.to_owned() + &"share_show_profile_image").as_str())
            .unwrap(),
        show_user_page: row
            .take((prefix.to_owned() + &"show_user_page").as_str())
            .unwrap(),
        theme_css: row
            .take((prefix.to_owned() + &"theme_css").as_str())
            .unwrap(),
        timezone: timezone,
        turn_off_advance_limits: row
            .take((prefix.to_owned() + &"turn_off_advance_limits").as_str())
            .unwrap(),
        ...
        username: row
            .take((prefix.to_owned() + &"username").as_str())
            .unwrap(),
        version_of: row
            .take((prefix.to_owned() + &"version_of").as_str())
            .unwrap(),
...
pub async fn log_user_in(pool: &Data<Pool>,email: String, password: String) -> LoginResult {
    ...
    match pool.get_conn().await {
        Ok(mut conn) => {
            ...
            //     }
            // }
            let found_user_result: Option<Row> = conn.exec_first("select id, banned, banned_reason from `users` where `email` like ? and password = ?", (email.to_owned(), encrypted_pass.to_owned())).await.unwrap();
            match found_user_result {
                Some(mut row) => {
