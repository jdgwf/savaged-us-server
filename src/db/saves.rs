...
fn _make_save_from_row(mut row: Row, with_cached_data: bool) -> SaveDBRow {
    ...
    if with_cached_data {
        ...
        match export_generic_json {
            Some(val_option) => match val_option {
                Ok(val) => {
    ...
    let base_file_name = save_type.to_string()
        + &"s/".to_owned()
